...
exports.Engine = exports.HashFlag = void 0;
...
var HashFlag;
(function (HashFlag) {
    HashFlag[HashFlag["exact"] = 0] = "exact";
    HashFlag[HashFlag["alpha"] = 1] = "alpha";
    HashFlag[HashFlag["beta"] = 2] = "beta";
})(HashFlag || (exports.HashFlag = HashFlag = {}));
class Engine {
    chess;
    fen;
    searchDepth;
    ...
    killerMove = [new Array(64).fill(null), new Array(64).fill(null)];
    ...
    constructor(options) {
        ...
        this.chess = new chess_js_1.Chess(this.fen);
    ...
    // Tranposition table
    hashTable = {};
    recordHash(score, depth, hashFlag, move) {
        // if (this.stable) return;
        const hash = (0, zobrist_1.genZobristKey)(this.chess).toString();
    ...
    // Move ordering
    getMovePrio(move, currentBoardHash) {
        let priority = 0;
        // Hash Move
        // const currentBoardHash = this.chess.fen();
        if (this.hashTable[currentBoardHash] &&
        ...
        if (!move.captured) {
            ...
            // History move
            priority += this.historyMove[move.color][evaluations_1.PIECE_NUM[move.piece]][move.to] || 0;
            return priority;
        }
        // For captures, we use MVV-LVA heuristic
        const attacker = move.piece;
        const victim = move.captured;
        priority += evaluations_1.mvv_lva[evaluations_1.PIECE_NUM[attacker]][evaluations_1.PIECE_NUM[victim]];
        return priority;
    }
    sortMoves(moves) {
        const currentBoardHash = (0, zobrist_1.genZobristKey)(this.chess).toString();
        return moves
    ...
    quiescence(alpha, beta) {
        ...
        this.nodes++;
        const evaluation = (0, evaluate_1.evaluateBoard)(this.chess);
    ...
    calculateExtensions(moves, inCheck) {
        let extensions = 0;
    ...
    negamax(depth, alpha, beta, extended) {
        ...
        let searchedMoves = 0, bestMoveSoFar;
        // Find the best move
        for (const move of possibleMoves) {
            ...
            if (searchedMoves === 0) {
                ...
                // Late move reduction
                if (searchedMoves >= this.lmrFullDepth &&
            ...
            if (score > alpha) {
                ...
                // Store history moves
                if (!move.captured) { // Only quiet moves
                    this.historyMove[move.color][evaluations_1.PIECE_NUM[move.piece]][move.to] += depth;
                }
                alpha = score;
